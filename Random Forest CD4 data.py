...
diff_CD4=pd.read_csv('./DiffGenExpres CD4')
diff_CD4.set_index('Gene Symbol', inplace=True) 
diff_CD4.head()
# Filtering out the samples which could not mapped to a gen symbol 
diff_CD4_filtered = diff_CD4[diff_CD4.index != '---']
diff_CD4_filtered.shape
diff_CD4_filtered.head()
# In order to simplify the training the column names were assigned as 'AML' or'healthy'
column_names=['AML','AML','AML','AML','AML','AML','AML','AML','AML','AML','healthy','healthy','healthy','healthy','healthy','healthy','healthy','healthy','healthy','healthy']
diff_CD4_filtered.columns = column_names
print(diff_CD4_filtered.head())
# Proving if there are NA values in the dataset
diff_CD4_filtered.isna().sum() # 0 NA values across the rows 

# Setting the labels
labels=['AML','AML','AML','AML','AML','AML','AML','AML','AML','AML','healthy','healthy','healthy','healthy','healthy','healthy','healthy','healthy','healthy','healthy']
